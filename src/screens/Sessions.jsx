...
export default function Sessions() {
  ...
  // ─── 保存（Firestore・即時） ──────────────────────────────
  // 入力のたびに自動保存。別のデバイスも onSnapshot でリアルタイム更新される。
  const save = (updated) => {
    setDoc(
      doc(db,'facilities',FACILITY_ID,'sessions',dateKey),
  ...
  // ─── 重複防止ヘルパー ────────────────────────────────────
  // 同コマ内で選択済みの職員IDを収集（自分自身は除く）
  const usedStaffIds = (slot, excludeCardIdx) =>
    slot.cards
      .filter((_, i) => i !== excludeCardIdx)
      .map(c => c.staffId)
      .filter(Boolean)

  // 同コマ内で選択済みの子どもIDを収集（自カード・自行は除く）
  const usedChildIds = (slot, cardIdx, excludeChildIdx) => {
    const ids = []
    slot.cards.forEach((c, ci) => {
      c.children.forEach((ch, chi) => {
        if (ci === cardIdx && chi === excludeChildIdx) return
        if (ch.childId) ids.push(ch.childId)
      })
    })
    return ids
  }
  ...
  // ─── カード操作 ──────────────────────────────────────────
  const addCard    = (si) => updateSlot(si, s => ({ ...s, cards: [...s.cards, emptyCard()] }))
  const removeCard = (si, ci) => updateSlot(si, s => ({
    ...s, cards: s.cards.length > 1 ? s.cards.filter((_,i) => i !== ci) : s.cards
  }))
      ...
      {slots.map((slot, si) => (
        <div key={si} style={{ background:C.card, borderRadius:20, padding:14, marginBottom:14, border:`1.5px solid ${C.border}` }}>
          ...
          {slot.cards.map((card, ci) => {
            // ★ このコマで選択済みの職員（自分以外）を除外
            const takenStaff    = usedStaffIds(slot, ci)
            // ★ このコマで選択済みの子ども（自カード・自行以外）はあとで行ごとに計算
            return (
              <div key={card.id || ci} style={{ background:C.bg, borderRadius:14, padding:12, marginBottom:10, border:`1.5px solid ${C.border}` }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10 }}>
                  ...
                  <select
                    ...
                    {staffList
                      .filter(x => !takenStaff.includes(x.id) || x.id === card.staffId)
                      .map(x => (
                ...
                {card.children.map((ch, chi) => {
                  ...
                  // ★ このコマで選択済みの子ども（自行を除く）
                  const takenChildren = usedChildIds(slot, ci, chi)
                  // 「さん」付きの表示名
                  const childLabel = ch.childName ? `${ch.childName}さん` : 'お子さん'
                  return (
                    <div key={chi} style={{ background:C.card, borderRadius:10, padding:10, marginBottom:7, border:`1px solid ${C.border}` }}>
                      <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:7 }}>
                        ...
                        <select value={ch.childId}
                          ...
                          {children
                            .filter(x => !takenChildren.includes(x.id) || x.id === ch.childId)
                      ...
                      <textarea autoComplete="off"
                        ...
                        placeholder={`${childLabel}のようす・気づき`}
                        rows={2}
